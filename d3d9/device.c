...
static HRESULT d3d9_device_get_swapchains(struct d3d9_device *device)
{
    ...
    if (!(device->implicit_swapchains = malloc(
...
static struct wined3d_vertex_declaration *device_get_fvf_declaration(struct d3d9_device *device, DWORD fvf)
{
    ...
    if (device->fvf_decl_size == device->fvf_decl_count)
    {
        ...
        fvf_decls = realloc(fvf_decls, sizeof(*fvf_decls) * (device->fvf_decl_size + grow));
        if (!fvf_decls)
        {
...
HRESULT device_init_d3d9(struct d3d9_device *device, struct d3d9 *parent, struct wined3d *wined3d,
        ...
        D3DPRESENT_PARAMETERS *parameters, D3DDISPLAYMODEEX *mode)
{
    ...
    swapchain_desc = malloc(sizeof(*swapchain_desc) * count);
    if (!swapchain_desc)
    {
    ...
    device->fvf_decls = malloc(0);//HeapAlloc(GetProcessHeap(), 0, 0);
    if (!device->fvf_decls){
