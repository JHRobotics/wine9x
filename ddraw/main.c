...
{
    t->entries = calloc(1, initial_size * sizeof(*t->entries));
    if (!t->entries)
    {
...
DWORD ddraw_allocate_handle(struct ddraw_handle_table *t, void *object, enum ddraw_handle_type type)
{
    ...
    if (!(t->entry_count < t->table_size))
    {
        ...
        struct ddraw_handle_entry *new_entries = realloc(t->entries, new_size * sizeof(*t->entries));
        if (!new_entries)
        {
...
/* exception handling in mingw */
#ifdef __MINGW32__
LPTOP_LEVEL_EXCEPTION_FILTER oldexcepfilter;

void __cdecl DirectDrawEnumerateExReturn(DWORD code);
void setMingwRecoveryPoint();

LONG WINAPI mingwExceptionFilter(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
	  DirectDrawEnumerateExReturn(E_INVALIDARG);
...
HRESULT WINAPI DirectDrawEnumerateExA(LPDDENUMCALLBACKEXA callback, void *context, DWORD flags)
{
    ...
    /* call custom handler and suppress exceptions */
#if defined _MSC_VER
    __try
    {
#elif defined __MINGW32__
    oldexcepfilter = SetUnhandledExceptionFilter(mingwExceptionFilter);
    do{
#else
    do{
#endif
    ...
    SetUnhandledExceptionFilter(oldexcepfilter);
#else
    } while(0);
#endif
...
#define WINED3D_DLLMAIN() if(DllMain_wined3d(inst, reason, reserved) == FALSE){return FALSE;}

#else
#define WINED3D_DLLMAIN() 
#endif

static HINSTANCE ddHinst = NULL;

HINSTANCE getDDHInst()
{
	return ddHinst;
}

typedef BOOL (WINAPI * GetModuleHandleExW_f)(DWORD, LPCWSTR, HMODULE *);
...
BOOL WINAPI DllMain(HINSTANCE inst, DWORD reason, void *reserved)
{
	...
	GetModuleHandleExW_f pGetModuleHandleExW = NULL;
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        #ifndef WINED3D_STATIC
        wine_debug_logs_init("dwine.log");
        #endif
        ...
        pGetModuleHandleExW = GetProcAddress(hKernel32, "GetModuleHandleExW");
...
#else
				if(pGetModuleHandleExW != NULL)
				{
	        if (!pGetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN,
	                (const WCHAR *)&ddraw_self, &ddraw_self))
	            ERR("Failed to get own module handle.\n");
	      }
	      else
	      {
	      	/* For Windows 9x this is bit hack - we just load self again  
	      	 *
	      	 */
	      	char sz[MAX_PATH];
					if (GetModuleFileName(inst, sz, MAX_PATH))
					{
					    LoadLibrary(sz);
					}
	      }
    ...
    case DLL_PROCESS_DETACH:
        ...
        if(!list_empty(&global_ddraw_list))
        {
            ...
            /* JH: application unloading DLL even there is active rendering, so STOP unloading? */
            return FALSE;
