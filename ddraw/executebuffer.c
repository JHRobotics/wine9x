	...
	switch (current->bOpcode) {
		...
		TRACE("TRIANGLE         (%d)\n", count);
                ...
                if (buffer->nb_indices < count * 3)
                {
                    ...
                    free(buffer->indices);
                    buffer->indices = malloc(sizeof(*buffer->indices) * buffer->nb_indices);
                }
	    ...
	    case D3DOP_MATRIXLOAD:
            ...
            case D3DOP_PROCESSVERTICES:
            {
                ...
                multiply_matrix_ddraw(&mat, &view_mat, &world_mat);
                multiply_matrix_ddraw(&mat, &proj_mat, &mat);
...
static HRESULT WINAPI d3d_execute_buffer_SetExecuteData(IDirect3DExecuteBuffer *iface, D3DEXECUTEDATA *data)
{
    ...
    free(buffer->vertex_data);
    buffer->vertex_data = calloc(1, nbvert * sizeof(D3DTLVERTEX));
        ...
        struct d3d_device *device, D3DEXECUTEBUFFERDESC *desc)
{
    ...
    if (!execute_buffer->desc.lpData && execute_buffer->desc.dwBufferSize)
    {
        ...
        execute_buffer->desc.lpData = calloc(1, execute_buffer->desc.dwBufferSize);
