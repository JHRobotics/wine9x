...
#ifdef LOG_TRACE
static unsigned char default_flags = (1 << __WINE_DBCL_ERR) | (1 << __WINE_DBCL_FIXME) | (1 << __WINE_DBCL_WARN) | (1 << __WINE_DBCL_TRACE);
#else
...
static char *get_temp_buffer_list[32] = {NULL};
static int get_temp_buffer_pos = 0;
...
static char *get_temp_buffer( size_t size )
{
    ...
    idx = get_temp_buffer_pos++ % (sizeof(get_temp_buffer_list)/sizeof(get_temp_buffer_list[0]));
    
    //printf("get_temp_buffer_list[%d]: %p (%d)\n", idx, get_temp_buffer_list[idx], size);
    
    if ((ret = realloc( get_temp_buffer_list[idx], size ))) get_temp_buffer_list[idx] = ret;
...
{
	char logfile[128];
	sprintf(logfile, "proc_%u_%s", GetCurrentProcessId(), logname);
	
	log_file = fopen(logfile, "wt");
}
