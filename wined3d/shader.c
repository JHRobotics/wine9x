        ...
        unsigned int resource_idx, unsigned int sampler_idx, unsigned int bind_idx)
{
    ...
    if (!map->size)
    {
        if (!(entries = malloc( sizeof(*entries) * 4)))
        {
        ...
        struct wined3d_shader_signature *output_signature, const DWORD *byte_code, DWORD constf_size)
{
    ...
    while (!fe->shader_is_end(fe_data, &ptr))
    {
        ...
        {
            struct wined3d_shader_lconst *lconst = malloc( sizeof(*lconst));
            float *value;
    ...
    else if (!input_signature->elements && reg_maps->input_registers)
    {
        ...
        if (!(input_signature->elements = malloc( sizeof(*input_signature->elements) * count)))
            return E_OUTOFMEMORY;
    ...
    else if (reg_maps->output_registers)
    {
        ...
        if (!(output_signature->elements = malloc( sizeof(*output_signature->elements) * count)))
            return E_OUTOFMEMORY;
...
static void shader_cleanup(struct wined3d_shader *shader)
{
    ...
    free(shader->signature_strings);
    shader->device->shader_backend->shader_destroy(shader);
    free(shader->reg_maps.constf);
    free(shader->reg_maps.sampler_map.entries);
        ...
        const struct wined3d_shader_signature *src, char **signature_strings)
{
    ...
    if (!(dst->elements = malloc( sizeof(*dst->elements) * dst->element_count)))
        return E_OUTOFMEMORY;
        ...
        void *parent, const struct wined3d_parent_ops *parent_ops)
{
    ...
    if (!(shader->signature_strings = malloc( total)))
        return E_OUTOFMEMORY;
