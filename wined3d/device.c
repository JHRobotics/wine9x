...
BOOL device_context_add(struct wined3d_device *device, struct wined3d_context *context)
{
    ...
    if (!device->contexts) new_array = malloc( sizeof(*new_array));
    else new_array = realloc(device->contexts,
...
void device_context_remove(struct wined3d_device *device, struct wined3d_context *context)
{
    ...
    new_array = realloc(device->contexts, device->context_count * sizeof(*device->contexts));
    if (!new_array)
    {
...
void CDECL wined3d_device_setup_fullscreen_window(struct wined3d_device *device, HWND window, UINT w, UINT h)
{
    ...
    device->style = GetWindowLongA(window, GWL_STYLE);
    device->exStyle = GetWindowLongA(window, GWL_EXSTYLE);
...
void CDECL wined3d_device_restore_fullscreen_window(struct wined3d_device *device, HWND window)
{
    ...
    {
        SetWindowLongA(window, GWL_STYLE, device->style);
        SetWindowLongA(window, GWL_EXSTYLE, device->exStyle);
    }
        ...
        struct wined3d_swapchain_desc *swapchain_desc)
{
    ...
    device->fb.render_targets = calloc(1,
    ...
    device->swapchains = malloc( device->swapchain_count * sizeof(*device->swapchains));
        ...
        unsigned int view_idx, struct wined3d_rendertarget_view *view, BOOL set_viewport)
{
    ...
    if (view && !(view->resource->usage & WINED3DUSAGE_RENDERTARGET))
    {
        ...
        //return WINED3DERR_INVALIDCALL;
        // JH: this will happen if surface hasn't flag DDSCAPS_VIDEOMEMORY, ignore the rest of code, but return success
        return WINED3D_OK;
    }
...
static HRESULT create_primary_opengl_context(struct wined3d_device *device, struct wined3d_swapchain *swapchain)
{
    ...
    swapchain->context = malloc( sizeof(*swapchain->context));
