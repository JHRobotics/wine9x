...
fail:
    ...
    free(object->changed.pixelShaderConstantsF);
    free(object->changed.vertexShaderConstantsF);
    free(object->contained_vs_consts_f);
    free(object->contained_ps_consts_f);
...
void state_cleanup(struct wined3d_state *state)
{
    ...
    free(state->vs_consts_f);
    free(state->ps_consts_f);
}
...
ULONG CDECL wined3d_stateblock_decref(struct wined3d_stateblock *stateblock)
{
    ...
    if (!refcount)
    {
        ...
        free(stateblock->changed.vertexShaderConstantsF);
        free(stateblock->changed.pixelShaderConstantsF);
        free(stateblock->contained_vs_consts_f);
        free(stateblock->contained_ps_consts_f);
        free(stateblock);
    }
        ...
        DWORD flags)
{
    ...
    if (!(state->vs_consts_f = calloc(1,
            4 * sizeof(float) * d3d_info->limits.vs_uniform_count)))
        return E_OUTOFMEMORY;

    if (!(state->ps_consts_f = calloc(1,
