...
static struct wined3d_context *swapchain_create_context(struct wined3d_swapchain *swapchain)
{
    ...
    newArray = malloc( sizeof(*newArray) * (swapchain->num_contexts + 1));
    if(!newArray) {
...
void wined3d_swapchain_activate(struct wined3d_swapchain *swapchain, BOOL activate)
{
    ...
    if(VirtualQuery(device, &meminfo, sizeof(struct wined3d_device)) == 0) return;
    if (meminfo.State != MEM_COMMIT || meminfo.Protect == PAGE_NOACCESS) return;
    /* JH: this HACK check accessibility of memory page with "device" struct...
     *     BUT this is very BAD idea, how solve race condition!
     *     If someone diagnose, how this lock properly, please do it and delete this hack!
     */
