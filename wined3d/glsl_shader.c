...
static void shader_glsl_dump_program_source(const struct wined3d_gl_info *gl_info, GLuint program)
{
    ...
    shaders = malloc( shader_count * sizeof(*shaders));
    if (!shaders)
    {
		...
		)
{
...
#ifdef VBOX_WITH_WINE_FIX_SHADERCLEANUP
	{
	/* JHFIX: some modif of orignal patch */
		struct glsl_context_data *ctx_data = context->shader_backend_data;
    	if (ctx_data->glsl_program == entry)
    	{
    ...
    free(entry->vs.uniform_f_locations);
    free(entry->ps.uniform_f_locations);
    free(entry);
}
        ...
        const struct wined3d_shader_reg_maps *reg_maps_out)
{
    ...
    set = calloc(1, sizeof(*set) * max_varyings);
        ...
        const struct ps_compile_args *args, const struct ps_np2fixup_info **np2fixup_info)
{
    ...
    if(shader_data->shader_array_size == shader_data->num_gl_shaders) {
        if (shader_data->num_gl_shaders)
        {
            ...
            new_array = realloc(shader_data->gl_shaders.ps,
        ...
        const struct vs_compile_args *args)
{
    ...
    if(shader_data->shader_array_size == shader_data->num_gl_shaders) {
        ...
        {
            new_size = shader_data->shader_array_size + max(1, shader_data->shader_array_size / 2);
            new_array = realloc(shader_data->gl_shaders.vs,
                    new_size * sizeof(*gl_shaders));
        }
        else
        {
        ...
        struct wined3d_shader *shader)
{
    ...
    if (!(shader_data->gl_shaders.gs = malloc( sizeof(*gl_shaders))))
    {
        ...
        struct shader_glsl_priv *priv, struct glsl_context_data *ctx_data)
{
    ...
    /* Create the entry */
    entry = malloc( sizeof(struct glsl_shader_prog_link));
    entry->id = program_id;
    entry->vs.id = vs_id;
    entry->gs.id = gs_id;
    entry->ps.id = ps_id;
        ...
        const struct fragment_pipeline *fragment_pipe)
{
    ...
    struct shader_glsl_priv *priv = calloc(1, sizeof(struct shader_glsl_priv));
    ...
    priv->stack = malloc( stack_size * sizeof(*priv->stack));
    if (!priv->stack)
    {
        ERR("Failed to allocate memory.\n");
        goto fail;
...
{
    return !!(context->shader_backend_data = calloc(1, sizeof(struct glsl_context_data)));
}
