...
{
#ifndef VBOX_WITH_WINE_FIX_MUTE_ERRORS
    ERR("Undefined state.\n");
#else
...
static void state_blend(struct wined3d_context *context, const struct wined3d_state *state, DWORD state_id)
{
    ...
    {
    	if(!(srcBlend == GL_SRC_COLOR && dstBlend == GL_SRC_ALPHA)) /* JHFIX: my hw denied this... */
    	{
...
static void sampler(struct wined3d_context *context, const struct wined3d_state *state, DWORD state_id)
{
    ...
    if (state->textures[sampler_idx])
    {
        ...
        gl_tex = wined3d_texture_get_gl_texture(texture, context->gl_info, texture->flags & WINED3D_TEXTURE_IS_SRGB);
...
static void vertexdeclaration(struct wined3d_context *context, const struct wined3d_state *state, DWORD state_id)
{
    ...
    if (!useVertexShaderFunction)
    {
        ...
        for (ii = 0; ii < MAX_TEXTURES; ++ii)
        {
            if (!isStateDirty(context, STATE_TRANSFORM(WINED3D_TS_TEXTURE0 + ii)))
                transform_texture(context, state, STATE_TEXTURESTAGE(ii, WINED3D_TSS_TEXTURE_TRANSFORM_FLAGS));
        }
        ...
        for(i = 0; cur[i].state; i++) {
            ...
            switch(handlers) {
                ...
                case 1:
                    ...
                    dev_multistate_funcs[cur[i].state] = malloc(sizeof(**dev_multistate_funcs) * 2);
