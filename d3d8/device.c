...
static DWORD d3d8_allocate_handle(struct d3d8_handle_table *t, void *object, enum d3d8_handle_type type)
{
    ...
    if (!(t->entry_count < t->table_size))
    {
        ...
        struct d3d8_handle_entry *new_entries = realloc(t->entries, new_size * sizeof(*t->entries));
        if (!new_entries)
        {
...
static struct d3d8_vertex_declaration *d3d8_device_get_fvf_declaration(struct d3d8_device *device, DWORD fvf)
{
    ...
    if (device->declArraySize == device->numConvertedDecls)
    {
        ...
        convertedDecls = realloc(convertedDecls,
        ...
        const struct wined3d_parent_ops **parent_ops)
{
    ...
    if (!(d3d_surface =  calloc(1, sizeof(*d3d_surface))))
        return E_OUTOFMEMORY;
...
HRESULT device_init_d3d8(struct d3d8_device *device, struct d3d8 *parent, struct wined3d *wined3d, UINT adapter,
        D3DDEVTYPE device_type, HWND focus_window, DWORD flags, D3DPRESENT_PARAMETERS *parameters)
{
    ...
    device->ref = 1;
    device->handle_table.entries =  calloc(1,
    ...
    device->decls = malloc(device->declArraySize * sizeof(*device->decls));
    if (!device->decls)
    {
